...
inline const std::unordered_map<int, std::string> WinsockInterface::_TABLE_OF_ERRORS =
{
                                                           ...
                                                           char* messageWithIP, char* buffer) const
{
    ...
    getpeername(socketForReceiving, DOUBLE_STATIC_CAST(SOCKADDR*, address), &addrlen);

    const int valRead	= recv(socketForReceiving, buffer, _MAXRECV, 0);
    const u_short port	= ntohs(address.sin_port);

    // Check if size of received data is less than max read constant.
    assert(valRead < _MAXRECV);
